...
if ( isset($_GET['import']) )
{
    ...
    function fix_special_char($a1)
    {
        if(!empty($a1)){
        $a1 = (iconv("ISO-8859-1", "UTF-8//TRANSLIT//IGNORE", $a1));
        }
    ...
    {
        if(!isset($filelayout['v_attributes_discounted'])) $filelayout['v_attributes_discounted'] = count($filelayout);
        if(!isset($filelayout['v_attributes_price_base_included'])) $filelayout['v_attributes_price_base_included'] = count($filelayout);
        if(!isset($filelayout['v_products_id'])) $filelayout['v_products_id'] = count($filelayout);
        if(!isset($filelayout['v_options_id'])) $filelayout['v_options_id'] = count($filelayout);
        if(!isset($filelayout['v_options_values_id'])) $filelayout['v_options_values_id'] = count($filelayout);

        //var_dump('<pre>', $filelayout); //die();
        ...
        while ($items = fgetcsv($handle, 0, $csv_delimiter, $csv_enclosure)) // read 1 line of data
        {
            ...
            $products_price_1 = 0;
            $options_values_price = (float)$items[$filelayout['v_options_values_price']];
            $price_prefix = ''; //$items[$filelayout['v_price_prefix']]

            $products_weight_1 = 0;
            $products_attributes_weight = (float)$items[$filelayout['v_products_attributes_weight']];
            $weight_prefix = ''; //$items[$filelayout['v_products_attributes_weight_prefix']]


            #/Get Product Id
            $sql_1 = "select products_id, products_price, products_weight from products where products_model='{$items[$filelayout['v_products_model']]}'";
            $result_1 = ep_4_query($sql_1);
            if ($row = mysql_fetch_array($result_1)) {
            $items[$filelayout['v_products_id']] = $row['products_id'];
            $products_price_1 = (float)$row['products_price'];
            $products_weight_1 = (float)$row['products_weight'];
            } //echo $sql_1;

            if(!isset($items[$filelayout['v_products_id']]))
            $items[$filelayout['v_products_id']] = '';

            //var_dump($filelayout['v_products_model'], $items[$filelayout['v_products_id']]); die();
            #-

            #/ Calculate Attribute additional price + weight
            if(($products_price_1>0) && ($options_values_price>0))
            {
                $price_diff = ($options_values_price-$products_price_1);
                $options_values_price = $price_diff;
            ...
            if(($products_weight_1>0) && ($products_attributes_weight>0))
            {
                $weight_diff = ($products_attributes_weight-$products_weight_1);
                $products_attributes_weight = $weight_diff;
            ...
            //var_dump($products_price_1, $price_diff); //die();
            #-

            #/Get Options Id
            $sql_1 = "select products_options_id from products_options where products_options_name='{$items[$filelayout['v_products_options_name']]}'";
            $result_1 = ep_4_query($sql_1);
            if ($row = mysql_fetch_array($result_1)) {
            $items[$filelayout['v_options_id']] = $row['products_options_id'];
            }

            if(!isset($items[$filelayout['v_options_id']]))
            $items[$filelayout['v_options_id']] = '';

            //var_dump($items[$filelayout['v_options_id']]); die();
            #-

            #/Get options_values_id
            $sql_1 = "select products_options_values_id from products_options_values where products_options_values_name='{$items[$filelayout['v_products_options_values_name']]}'";
            $result_1 = ep_4_query($sql_1);
            if ($row = mysql_fetch_array($result_1)) {
            $items[$filelayout['v_options_values_id']] = $row['products_options_values_id'];
            }

            if(!isset($items[$filelayout['v_options_values_id']]))
            $items[$filelayout['v_options_values_id']] = '';

            //var_dump($items[$filelayout['v_options_values_id']]); die();
            #-
			...
			if ($row = mysql_fetch_array($result))
            {
				if(!isset($items[$filelayout['v_attributes_discounted']]))
                $items[$filelayout['v_attributes_discounted']] = 0;

                if(!isset($items[$filelayout['v_attributes_price_base_included']]))
                $items[$filelayout['v_attributes_price_base_included']] = 1;
				...
				$sql = "UPDATE ".TABLE_PRODUCTS_ATTRIBUTES." SET
					...
					products_attributes_weight_prefix = '".$weight_prefix."',
					...
					color_sizes                       = '".$items[$filelayout['v_color_sizes']]."',
            ...
            else
            {
                ...
                #/Insert
                $sql = "INSERT INTO ".TABLE_PRODUCTS_ATTRIBUTES." SET
					options_values_price              = '".$options_values_price."',
					price_prefix                      = '".$price_prefix."',
					products_options_sort_order       = '".$items[$filelayout['v_products_options_sort_order']]."',
					product_attribute_is_free         = '".$items[$filelayout['v_product_attribute_is_free']]."',
					...
					options_id                        = '".$items[$filelayout['v_options_id']]."',
					options_values_id                 = '".$items[$filelayout['v_options_values_id']]."'";
				//die("<pre>".$sql);
                $result = ep_4_query($sql);
    ...
    //  temporary solution here... 12-06-2010
    if ( ( strtolower(substr($file['name'],0,15)) <> "categorymeta-ep") && ( strtolower(substr($file['name'],0,7)) <> "attrib-"))
    {
	   ...
	   while ($items = fgetcsv($handle, 0, $csv_delimiter, $csv_enclosure))// read 1 line of data
       {
    		...
    		while ( $row = mysql_fetch_array($result) ) // chadd - this executes once?? why use while-loop??
            {
    			...
    			foreach ($langcode as $key => $lang)
                {
                    ...
                    // create variables (v_products_na me_1, v_products_na me_2, etc. which corresponds to our column headers) and assign data
    		...
    		foreach ($langcode as $lang)
            {
                ...
                { // do for each language in our upload file if exist
            ...
            if ( strtolower(substr($file['name'],0,14)) == "pricebreaks-ep") {
    			...
    			if (isset($v_products_name))
                {
    				foreach( $v_products_name as $key => $name)
                    {
                        ...
                        {
                            $pn_1 = fix_special_char($name);
                            $pd_1 = fix_special_char($v_products_description[$key]);
                        ...
                        else // already in the description, update it
                        {
                            ...
                            $sql = "UPDATE ".TABLE_PRODUCTS_DESCRIPTION." SET
    							products_name        ='".addslashes($pn_1)."',
    							products_description ='".addslashes($pd_1)."',";
    						if ($ep_supported_mods['psd'] == true) {
